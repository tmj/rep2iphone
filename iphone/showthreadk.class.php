...
EOID;
    ...
    //iPhone用　別窓変換。通勤ブラウザを書き換え
        $tsukin_link = '';
        if ($_conf['k_use_tsukin']) {
            $tsukin_link = '<a href="' .($in_url) . '" target="_blank">窓</a>';
        }
...
//$r = $ext_pre . '<a href="' . $in_url . '">URL</a>';
 ...
 //           $attributes .= " onmouseout=\"hideResPopUp('q{$qnum}of{$this->thread->key}')\"";
        ...
        return "<a href=\"{$read_url}{$_conf['k_at_a']}\"{$attributes}>{$qsign}{$appointed_num}</a>";
    }
    ...
    function plugin_link2chSubject($url, $purl, $str)
    {
        ...
        if (preg_match('{^http://(\\w+\\.(?:2ch\\.net|bbspink\\.com))/([^/]+)/$}', $url, $m)) {
            ...
            return "<a href=\"{$url}\">p2ch</a> [<a href=\"{$subject_url}{$_conf['k_at_a']}\">板をp2で開く</a>]";
        }
        return FALSE;
    }
